...
function occursOn(ev, day, base) {
  ...
  if (
    Array.isArray(ev.excluded_dates) &&
    ev.excluded_dates.some((x) => String(x).slice(0, 10) === wallDateStr(dd))
  )
    return false;
  ...
  switch (ev.recurrence_type) {
    ...
    case "weekly": {
      const days = (ev.recurrence_days || [])
        .map((x) => WEEKDAY[String(x).toLowerCase()])
        .filter((n) => n != null);
      if (days.length) {
    ...
    case "monthly":
      return (
        dd.getDate() === d0.getDate() &&
        ((dd.getFullYear() - d0.getFullYear()) * 12 + dd.getMonth() - d0.getMonth()) % interval ===
          0
      );
    default:
      return false;
...
function expandEvents(events, rangeStart, rangeEnd) {
  ...
  for (const ev of events) {
    ...
    const from =
      startOfDay(rangeStart) > startOfDay(base) ? startOfDay(rangeStart) : startOfDay(base);
...
export default function CalendarPage() {
  ...
  const newSprint = () => {
    if (!activeSpaceId) {
      toast.error("Open a space first to create a sprint.");
      return;
    }
  ...
  return (
    <div className="-mx-2 -my-3 flex h-[calc(100dvh-3.5rem)] bg-background sm:-mx-4 sm:-my-6 lg:-mx-8">
      {/* Calendar side panel */}
      <aside className="hidden md:flex w-64 shrink-0 flex-col gap-5 border-r bg-card/40 p-4 overflow-y-auto scrollbar-thin">
        <DropdownMenu>
          ...
          <DropdownMenuContent align="start" className="w-[14.5rem]">
            ...
            <DropdownMenuItem onClick={() => setImportScheduleOpen(true)}>
              Import schedule (AI)
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
        ...
        <div>
          ...
          <div className="space-y-1">
            {ranges.map((r) => {
              ...
              return (
                ...
                >
                  ...
                  {r.active && (
                    <span className="ml-auto text-[10px] font-semibold text-emerald-600 uppercase">
                      Now
                    </span>
                  )}
                </div>
            ...
            {ranges.length === 0 && (
              <p className="px-2.5 text-xs text-muted-foreground">No sprints yet.</p>
            )}
          </div>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="min-h-16 border-b flex flex-wrap items-center px-2 py-2 gap-2 sm:px-4 sm:gap-3">
          <Button
            variant="outline"
            size="sm"
          ...
          <div className="flex items-center">
            <Button size="icon" variant="ghost" onClick={() => setCursor(addDays(cursor, -30))}>
          ...
          <h2 className="min-w-0 flex-1 text-center text-base font-semibold sm:ml-1 sm:text-left sm:text-lg">
            {cursor.toLocaleString("en", { month: "long", year: "numeric" })}
          </h2>
          <div className="order-last flex w-full rounded-lg border bg-muted/40 p-0.5 sm:order-none sm:ml-auto sm:w-auto">
            {VIEWS.map((v) => (
              <button
                key={v}
                onClick={() => setView(v)}
                className={cn(
                  "flex-1 rounded-md px-2 py-1 text-sm transition-colors sm:flex-none sm:px-3",
                  view === v
...
function SprintBar({ sprint }) {
  ...
  const day = Math.min(
    total,
    Math.max(1, Math.round((dayMs(new Date()) - dayMs(sprint.from)) / 86400000) + 1),
  );
  const fmt = (d) => d.toLocaleDateString("en-GB", { day: "numeric", month: "short" });
  return (
    <div className="mx-2 my-2 flex flex-wrap items-center gap-x-3 gap-y-1 rounded-lg bg-emerald-50 px-3 py-2 text-xs sm:mx-4 sm:text-sm">
    ...
    <div className="h-full min-w-0 flex flex-col">
      <div className="grid grid-cols-7 border-b bg-muted/20 text-[11px] font-medium uppercase tracking-wide text-muted-foreground">
        {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((d) => (
          <div key={d} className="px-1 py-2 text-center text-[10px] sm:px-2 sm:text-[11px]">
            {d}
          </div>
        ))}
      </div>
        ...
        {cells.map((d, i) => {
          ...
          return (
            <div
              ...
              className={cn(
                "group min-h-[76px] border-r border-b border-border/60 p-1 overflow-hidden cursor-pointer transition-colors hover:bg-muted/40 sm:min-h-[96px] sm:p-1.5",
            ...
            >
              <div
                className={cn("mb-1 flex items-center justify-center", otherMonth && "opacity-60")}
              >
                <span
                  className={cn(
                    "grid h-5 min-w-5 place-items-center rounded-full px-1 text-[10px] sm:h-6 sm:min-w-6 sm:text-xs",
              ...
              <div className="space-y-1">
                {dayTasks.slice(0, 2).map((t) => (
                  <button
                    ...
                    className="block w-full truncate rounded px-1 py-0.5 text-left text-[9px] text-white sm:px-1.5 sm:text-[11px]"
                ...
                {dayEvents.slice(0, 3).map((e) => (
                  ...
                  >
                    {e.all_day
                      ? e.title
                      : `${parseWall(e.start_time).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })} ${e.title}`}
                  </button>
                ))}
